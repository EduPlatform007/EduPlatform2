  ...
  function overrideUpdateCompleteButton() {
    if (typeof window.updateCompleteButton === 'function') {
      ...
      window.updateCompleteButton = function(lessonNum) {
        ...
        // Не делаем никаких автоматических изменений состояния кнопки
        // Пусть оригинальная функция решает, активировать кнопку или нет
        console.log('Вызвана оригинальная функция updateCompleteButton для урока ' + lessonNum);
      };
    }
  }
  
  // Автоматически исправляем только кнопки проверки тестов каждые 3 секунды
  setInterval(function() {
    // Активируем только кнопки проверки тестов, чтобы они не блокировались
    document.querySelectorAll('.quiz-container button.check-btn').forEach(function(button) {
      if (button.disabled) {
        button.disabled = false;
        console.log('Кнопка проверки теста активирована');
      }
    ...
    // Активируем кнопки проверки практики
    document.querySelectorAll('.practice-container button.run-code-btn').forEach(function(button) {
      if (button.disabled) {
        button.disabled = false;
        console.log('Кнопка проверки практики активирована');
      }
    });
    
    // Не трогаем кнопку завершения урока - пусть её активирует только оригинальная функция
    // при условии успешного выполнения всех заданий
    console.log('Проверка и активация кнопок проверки завершена');
  }, 3000);
